...
public Action Command_ForceEnter_Saferoom( int client, int args )
{
	...
	if( type == 0 )
	{
		...
		{
			g_CMClient[client].bStateJump = true;
...
public void OnMapStart()
{
	...
	g_EMEntity.iBloodSprite = PrecacheModel( SPR_BLOOD );
...
{
	g_EMEntity.iSensorType[0]		= -1;
	g_EMEntity.iSensorType[1]		= -1;
...
public void Event_PlayerRescued( Event event, const char[] name, bool dontBroadcast )
{
	...
	if ( Survivor_IsValid( client ))
	{
		...
		{
			PrintReviveMessage( client );
		}
		PrintTextToServer( "Survivor Rescue", g_bCvar_IsDebugging );
	}
}
...
public void Event_PlayerReplace( Event event, const char[] name, bool dontBroadcast )
{
	...
	{
		// player takeover bot
		if( strcmp( name, "bot_player_replace", false ) == 0 )
		{
		...
		// bot takeover player
		else if( strcmp( name, "player_bot_replace", false ) == 0 )
		{
...
public void Event_Defibrillator( Event event, const char[] name, bool dontBroadcast )
{
	...
	{
		if( strcmp( name, "defibrillator_begin", false ) == 0 )
		{
			g_CMClient[subject].bIsUseDefib = true;
			PrintTextToServer( "Defibrillator Begin", g_bCvar_IsDebugging );
		}
		else if( strcmp( name, "defibrillator_used_fail", false ) == 0 || strcmp( name, "defibrillator_interrupted", false ) == 0 )
		{
			g_CMClient[subject].bIsUseDefib = false;
			PrintTextToServer( "Defibrillator Fail/Interrupted", g_bCvar_IsDebugging );
		}
		else if( strcmp( name, "defibrillator_used", false ) == 0 )
		{
			g_CMClient[subject].bIsUseDefib = false;
			if( !IsFakeClient( subject ))
			{
				PrintReviveMessage( subject );
			}
			PrintTextToServer( "Defibrillator Used", g_bCvar_IsDebugging );
		}
	}
}
...
void SetClientRoom( int client, int room )
{
	...
	if( g_CMClient[client].bStateJump )
	{
		g_CMClient[client].bStateJump = false;
	}
	else
	{
		// door touched is not equal to the previous door touched
		if( g_CMClient[client].iStateRoom != room )
		{
			switch( room )
			{
				...
				{ 
					...
					if( g_CMClient[client].iStateRoom == ROOM_STATE_SPAWN )
					{
						StopBurningSound( client );
					...
					else if( g_CMClient[client].iStateRoom == ROOM_STATE_RESCUE )
					{
				...
				case ROOM_STATE_RESCUE:
				{
					...
					if( g_CMClient[client].iStateRoom == ROOM_STATE_OUTDOOR || g_CMClient[client].iStateRoom == ROOM_STATE_SPAWN )
					{
						StopBurningSound( client );
					}
			...
			g_CMClient[client].iStateRoom = room;
		}
	}
}
...
public void EVENT_PlayerSpawn( Event event, const char[] name, bool dontBroadcast )
{
	...
	if ( client > 0 && client <= MaxClients && IsClientInGame( client ))
	{
		if( !IsFakeClient( client ))
		{
			PrintTextToServer( "Player Spawn", g_bCvar_IsDebugging );
		}
...
void CreateSaferoomSensor( int client )
{
	...
	if( g_EMEntity.iDoor_Spawn != -1 )
	{
		...
		g_EMEntity.iSensorType[SENSOR_ENTER] = Create_SensorModel( buf, ang, g_sDummyModel[MDL_SENSOR] );
		if( g_EMEntity.iSensorType[SENSOR_ENTER] != -1 )
		{
			SDKHook( g_EMEntity.iSensorType[SENSOR_ENTER], SDKHook_StartTouch, OnDoorSensorTouched );
			SDKHook( g_EMEntity.iSensorType[SENSOR_ENTER], SDKHook_EndTouch, OnDoorSensorTouched );
		}
		...
		g_EMEntity.iSensorType[SENSOR_EXIT] = Create_SensorModel( buf, ang, g_sDummyModel[MDL_SENSOR] );
		if( g_EMEntity.iSensorType[SENSOR_EXIT] != -1 )
		{
			SDKHook( g_EMEntity.iSensorType[SENSOR_EXIT], SDKHook_StartTouch, OnDoorSensorTouched );
			SDKHook( g_EMEntity.iSensorType[SENSOR_EXIT], SDKHook_EndTouch, OnDoorSensorTouched );
		}
...
public Action OnDoorSensorTouched( int entity, int client )
{
	...
	if( entity == g_EMEntity.iSensorType[SENSOR_ENTER] )
	{
		SetClientRoom( client, ROOM_STATE_SPAWN );
	}
	else if( entity == g_EMEntity.iSensorType[SENSOR_EXIT] )
	{
...
void StartGlobalTimer()
{
	...
	else if( g_EMEntity.iDoor_Spawn == -1 && g_EMEntity.iDoor_Rescue == -1 )
	{
		PrintTextToServer( "Map Error, no saferoom door found!!", g_bCvar_IsDebugging );
	}
}
...
public Action Timer_GlobalDamage( Handle timer )
{
	...
	if( g_EMEntity.bIsRound_Finale || g_EMEntity.bIsRound_End )
	{
		...
		PrintTextToServer( "Timer damage terminated for finale/round end", g_bCvar_IsDebugging );
	...
	for( int i = 1; i <= MaxClients; i++ )
	{
		...
		{
			// is damage shifted to rescue saferoom?
...
void Create_DamageEffect( int victim, int attacker, int damage )
{
	...
	SetupBloodEffect( victim, g_EMEntity.iBloodSprite, g_iCvar_BloodColor );
...
void PrintReviveMessage( int client )
{
...
void ToggleGlowEnable( int entity, int color[3], bool enable )
{
	int  m_glowtype = 0;
	int  m_glowcolor = 0;
